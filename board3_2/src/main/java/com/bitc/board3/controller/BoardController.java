...
//  HTTP URI를 통해 자원을 명시하고 HTTP Method를 통해 자원에 대한 CRUD 명령을 적용하는 것

//  URI : Uniform Resource Indentifier 의 줄임말(자원 식별자)
...
//  @RequestMapping 어노테이션 사용 시 URI를 value 속성을 상요하여 설정하고, method 속성을 사용하여 GET, POST, PUT, DELETE를 설정
//  @GetMapping, @PostMapping, @PutMapping, @DeleteMapping 를 사용하여 구분해도 됨

//  전체 URL 생성 방법 :
//  서버URL(localhost):포트번호(3306)/기본 주소/추가주소 or 게시물 번호/게시물 번호
...
//  @PathVariable : 지정한 자원의 이름과 Controller가 가지고 있는 메소드의 매개변수 명을 매칭시켜주는 어노테이션,
//  해당 자원의 데이터를 가져와서 메소드의 매개변수에 저장함
