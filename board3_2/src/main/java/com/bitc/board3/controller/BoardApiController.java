...
// 리액트 사용 시 프론트엔드 부분과 백엔드 부분이 완전히 분리되어 있기 때문에 데이터를 주고받기 위해서 ajax 통신이 필요함
@RestController
@RequestMapping("/board3/api")
public class BoardApiController {
//  RESTful API : REST 방식과 동일한 형태로 HTTP 프로토콜을 사용하지만,
//  클라이언트로 전달하는 데이터가 View를 제외한 데이터만 전달하는 방식
//  예전에는 클라이언트가 컴퓨터 밖에 없었기 때문에 웹브라우저가 클라이언트의 기능을 담당했음
//  서버의 기능이 웹 서버의 기능만 주로 사용되었기 때문에 클라이언트로 전달하는 데이터가 View와 데이터 모두 필요한 형태였음
//  현재는 클라이언트가 다양한 디바이스로 동작하기 때문에 View 부분은 웹 브라우저가 반드시 필요한 것은 아니게 되어서
//  데이터만 전달하는 방식으로 변경되었음
